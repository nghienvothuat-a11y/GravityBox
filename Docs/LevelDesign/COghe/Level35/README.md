# COghe — Màn 35 — Giữ rồi trả tự do

## 1. Định danh, phạm vi và trạng thái

ID đề xuất `venom.origin.35` · vị trí chơi **35** · Giữ rồi trả tự do. Người thiết kế: Codex, 21/09/2026. **Nháp v1** theo yêu cầu Mrk; không tự ghi là đã duyệt. Hồ sơ và phác thảo mới, không thay scene cũ. Chưa có scene/definition/builder mới; chưa kiểm chứng Editor/Mac/mobile.
...
Dùng kỹ năng bò/leo/luồn/đẩy/kéo đã có, cùng cắt/tụ và giao việc cho phần cơ thể. Vai trò: luyện/kết hợp thêm quan hệ so với màn 34. Kết quả chỉ ghi hoàn tất màn, không cấp buff hoặc mở lại Nhà. Điểm mới là quan hệ cơ quan trong bố cục này, không phải kỹ năng có thể mua.

## 3. Phác thảo, hình học, camera và thao tác

![Màn 35 — Giữ rồi trả tự do](mockup-v1.png)

Hai khoang trái/phải, vách kín có cửa D ở sàn. Trái có dao và nút A ở xa D. A giữ cơ cấu ly hợp nâng D; mất tải trước khi chốt thì D hồi đóng chậm, chống kẹp. Phải có tay L gắn chốt cơ khí giữ D ở vị trí mở, đồng thời rút khóa nắp E của lỗ cuối. E có tay kéo thường; vùng tụ nằm trong khoang phải trước E. L chỉ ở phía phải và chỉ bám được khi đã qua D.
...
**Sửa sai:** Nhả A sớm: phần ở phải ở yên trên bệ an toàn, phần trái trở lại A. Nếu D chưa mở đủ để cài L, mở lại rồi kéo. Tụ sớm thì về dao cắt lại. Chống kẹp không tự tạo tiến độ thắng.
...
**Quan hệ:** `A có tải → D mở tạm; người qua D kéo L → D chốt + E mở khóa; tụ → kéo E → thoát`.

Dao, cảm biến tải và cửa dùng lại; bổ sung liên kết spring-return door có chống kẹp + chốt thao tác được từ bên kia. Khác cửa phanh giữ vô thời hạn.
...
D mở tạm có biểu tượng lò xo; sau kéo L, ngàm chốt cài thấy rõ và trạng thái đổi sang chốt giữ. Đèn A chỉ theo tải, không giả sáng sau khi rời.
...
Dự kiến **4 cụm quyết định**, tối đa **2 vai trò cùng lúc** trong lời giải mẫu; đây không phải số cú chạm tối thiểu. Mục tiêu lần đầu 4–5 phút, **chưa đo**. Độ chính xác/thời điểm giữ thấp; cho phép dừng quan sát giữa các bước.

Rủi ro riêng: Không dùng phanh giữ D mở khi nhả A, vì một cơ thể sẽ bỏ nút rồi đi qua. Đo khoảng cách A–D, tốc độ hồi và tầm giãn để chặn bypass; không đòi đúng hai group-ID.
...
- Không dùng phanh giữ D mở khi nhả A, vì một cơ thể sẽ bỏ nút rồi đi qua. Đo khoảng cách A–D, tốc độ hồi và tầm giãn để chặn bypass; không đòi đúng hai group-ID.
...
Vị trí 35/ID 35 là đề xuất mới, chưa đăng ký catalog, scene, save hoặc build list; giữ nguyên các ID cũ. Khi tích hợp kiểm tra mở tiếp/replay/save idempotent, mất điện giữa lưu và ăn mừng; không cấp Nhà lại. Màn 40 kết thúc chương mới, chưa tự đặt màn 41.
...
Phần trên lưu thiết kế minh họa v1. Màn 35 hiện đã được dựng thành `Assets/_Game/Venom/Campaign30/COgheOrigin35.unity`, đăng ký campaign40 màn với ID ổn định `venom.origin.35`. Các khoảng hở, tay nắm và góc nhìn được hiệu chỉnh qua mô phỏng vật lý; quan hệ cơ quan và thứ tự giải giữ theo concept.

Luồng đã chạy trong app macOS: **Chia hai, giữ A cho phần kia qua D kéo L, hợp thể rồi kéo E và thoát.** Hợp thể trước khi ra cửa; ảnh trước thoát ghi1 phần/0 hạt thoát, ảnh thắng ghi1 phần/32 hạt thoát; không thua. Kiểm tra reset sau thắng và chọn các điều khiển nhìn thấy trong trạng thái đầu màn đều đạt.

Toàn bộ suite: **341/341 PlayMode,8/8 EditMode**. Cả10 lời giải đạt trong bản macOS Development thực tế; chưa đo hiệu năng hay chơi chạm tay trên thiết bị mobile. [Hồ sơ triển khai chung](../CHAPTER_31_40_IMPLEMENTATION.md) ghi trạng thái nguồn, các điều chỉnh và giới hạn kiểm chứng. Bằng chứng màn này: `Artifacts/chapter40-player-proof/20260921T135858138Z/level-35.json`, `level-35-open.png` và `level-35-won.png`.

## Rà đường đi và phục hồi — 21/09/2026

Góc nhìn khoang 2 nhìn từ phía lỗ để nắp E đã trượt không chắn cú chạm thoát. Kiểm tra bỏ A khi phần kia đã qua D, giữ A lại, chốt L rồi hợp thể và thoát.
