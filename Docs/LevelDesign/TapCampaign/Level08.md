# COghe — coghe.tap.v1.08 — Chia việc đổi khớp
...
ID `coghe.tap.v1.08`, thứ tự 8, scene `Assets/_Game/Venom/TapCampaign/COgheTap08.unity`, definition `TapCampaign/Definitions/Tap08.asset`. Builder `COgheTapCampaignBuilder.cs`. Phiên bản 1. Trạng thái: **Prototype — gameplay/input đã qua kiểm chứng Mac**; xem báo cáo chapter để biết kết quả thực. Chưa có novice/mobile evidence. Không thay ID hoặc completion của Origin/01–05.

## 2. Mục tiêu và tiến trình

Kết hợp selector và phân vai. B cần cả cam ở chốt 2 lẫn tải P. P cấp ly hợp nguồn liên tục cho đến khi cửa đã chốt.
...
```text
sau z=+.30
  pad x=-.28,z=.08    motor x=-.15,z=.13 -- A/B x=-.04 -- đầu ra -- nắp thoát x=.25,z=.18
...
Lời giải cho tác giả/test (runtime không đọc): **Chia → giữ P → A: 1→2 → bật B → A: 2→3→4 → cửa chốt → nhả P → nhập → thoát.**
...
Danh sách TapCampaign gồm 01–10, mỗi ID riêng, không đổi save schema `venom.origin.v2`. Nút tiếp theo đi đúng Tap09. Origin được giữ để hồi quy. Bản build riêng `Builds/COgheTapChapter`. Kết luận: lời giải/screen input và hồi quy đã qua; Mac 10/10, full PlayMode 382/382, EditMode 8/8. Novice/device là bước tiếp theo của người dùng.
