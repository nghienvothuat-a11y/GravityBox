# Kế hoạch 16 màn — tài liệu lịch sử

Đã được thay thế bởi yêu cầu prototype ba hộp để kiểm chứng cảm giác bi thép. Không dùng danh sách màn hoặc mốc nghiệm thu dưới đây làm phạm vi hiện tại. Xem [kế hoạch đang áp dụng](../IMPLEMENTATION_PLAN.md).
...
Chi tiết kiến trúc hiện tại: [ARCHITECTURE.md](../ARCHITECTURE.md); các quyết định trước được giữ trong [DECISIONS.md](../DECISIONS.md).
...
Ưu tiên nguyên nhân vật lý nhìn thấy được, hình học/contact, khối lượng, ma sát, quán tính và trọng lực thế giới. [Thiết kế nắp rơi](PHYSICS_DESIGN_LID.md) từng là chuẩn cho cơ chế mới, có ưu tiên so với các công tắc/tween quy ước của baseline GDD. L06 được chuyển trước thành nắp tự do; các tín hiệu, one-way gate và impulse pad cũ được ghi để rà soát. Phạm vi ba hộp hiện tại đã đưa toàn bộ các cơ chế này ra ngoài catalog.
