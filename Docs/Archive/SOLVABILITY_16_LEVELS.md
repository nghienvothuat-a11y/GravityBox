# Bằng chứng khả giải 16 màn — tài liệu lịch sử

Các route bên dưới thuộc baseline trước khi đổi sang ba hộp với kích thước vật lý mới. Không dùng chúng để xác nhận prototype bi thép hiện tại. Xem [phạm vi kiểm chứng hiện tại](../SOLVABILITY.md).
...
Dữ liệu gốc: [VERIFIED_ROUTES.json](../VERIFIED_ROUTES.json). Ở baseline cũ, test All16Levels_HaveReproduciblePhysicsOnlySolveRoutes bắt buộc replay chương trình xoay đã lưu và fail nếu không giải được trong ngân sách. Với L06, `aimAtExit` là policy kiểm thử được khai báo tường minh trong JSON; không phải hỗ trợ tự lái trong game. Khi author đặt `GRAVITYBOX_RECORD_SOLUTIONS=1`, fixture cũ tìm kiếm có seed/giới hạn và ghi lại route mới. Fixture này đã được thay bằng kiểm tra ba hộp; JSON hiện chỉ là dữ liệu lịch sử.
