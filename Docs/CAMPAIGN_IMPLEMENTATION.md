...
Dùng Unity **6000.3.19f1**. Menu **Gravity Box → Campaign → Generate 100 Levels** tạo lại từ JSON và các builder; commit/save thay đổi author trước khi regenerate. CLI `-executeMethod GravityBox.Editor.CampaignBuilder.GenerateAndAudit` chạy author và audit. `bash Tools/verify.sh` chạy EditMode và PlayMode; `bash Tools/build.sh macOS` và `bash Tools/build.sh Android` đều build `Campaign.unity` kèm khả năng chuyển sang Lab. Android xuất Development APK IL2CPP/ARM64, min API 26 tại `Builds/Android/GravityBox-Campaign100.apk`; [manifest, hash và phạm vi kiểm tra](Verification/Campaign100/Android/README.md). iOS chưa chuyển sang pipeline campaign.
