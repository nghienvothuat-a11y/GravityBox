...
namespace GravityBox.Venom
{
    ...
    public sealed class VenomSurface : MonoBehaviour
    {
        ...
        private readonly Vector3[] points = new Vector3[CohesiveOrganism.ParticleCount];
        ...
        public void Initialize(CohesiveOrganism source)
        {
        ...
        public void Rebuild(bool interpolate = true)
        {
            ...
            for (int i = 0; i < CohesiveOrganism.ParticleCount; i++)
            {
                ...
                if(count>0) BuildFragment(count);
            }
        ...
        {
            float support=organism.Profile.SkinSupport, threshold=organism.Profile.SkinThreshold;
            Vector3 min=points[0], max=points[0];
            for(int i=1;i<count;i++){min=Vector3.Min(min,points[i]);max=Vector3.Max(max,points[i]);}
            min-=Vector3.one*support;max+=Vector3.one*support;
            ...
            for(int p=0;p<count;p++)
            {
                ...
                for(int x=Mathf.Max(0,(int)q.x-reach);x<=Mathf.Min(nx-1,(int)q.x+reach);x++)
                {
                    ...
                    int index=x+nx*(y+ny*z);field[index]+=s*s*s;gradient[index]+=d*(6*s*s/h2);
                }
            }
