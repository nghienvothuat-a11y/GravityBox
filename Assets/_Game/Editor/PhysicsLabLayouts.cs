    ...
    internal static class PhysicsLabLayouts
    {
        public static PhysicsLabLayout[] All()
        {
            return new[]
            {
                ...
                Layout(ContainerShape.LionHead, "The lion's mouth", "Leave the mane. Roll between the eyes, around the muzzle and into the green mouth.",
                    LionHeadBuilder.Outline(), V(-.23f,.22f), V(0,-.215f),
                    solution: "A dry bronze-and-glass lion-head container under Earth gravity. Its jagged mane and ears are the real enclosure boundary. The bevelled eyes, nose and muzzle are fixed physical obstacles. From the upper-left mane, move to the forehead, pass between the eyes, turn around either cheek and reach the circular mouth exit. Face colouring and mane inlays are optical decoration, not invisible walls. The same steel sphere, contacts and rotation controls are used."),
                Layout(ContainerShape.CooperativeBox, "Two to escape", "Hold cyan A to let B cross. B presses amber to release A. Get BOTH balls out.",
                    new[] { V(-.32f,-.32f), V(.32f,-.32f), V(.32f,.32f), V(-.32f,.32f) }, V(-.20f,.25f), V(0,-.278f),
                    solution: "Two steel balls share one rotating box and Earth gravity. Cyan A holds a spring plunger while amber B crosses the right gate. B then compresses the retaining plunger, which holds both powered shutters open. Retrieve A from its holding pocket and guide both spheres to the one round exit. Shutters remain physical colliders throughout their motion. The level only completes after both full spheres have left."),
                Mechanism(ContainerShape.GravityBridge,"Build your own bridge","Park the ball. Tilt to seat the hinged bridge, then cross the gap.",
                    "Keep the steel ball in the holding bay while gravity folds the bridge into its receiving jaw. Actual receiver contact engages an ideal mechanical retaining catch at the reached angle. Cross the physical bridge into the exit bay. Reset releases the catch and restores the folded bridge."),
                Mechanism(ContainerShape.BalanceMachine,"Two balls, one balance","A has the longer lever arm. Lift B, seat the catch, then free BOTH balls.",
                    "Two equal steel balls load different lever arms. The far ball supplies the torque that raises the near ball and aligns its lane with the receiving ledge. A contact-operated catch retains the seated lever, allowing both balls to leave their lanes and reach the round exit. No powered lift or identity-specific ball forces."),
                Mechanism(ContainerShape.NestedCage,"A box within a box","The inner cage hangs under gravity. Tilt past its stop to line up the mouth.",
                    "The counterweighted inner cage rotates independently on its bearing. Tilt the outer enclosure past the visible bearing stop so the inner floor slopes towards its only mouth. Transfer the ball onto the outer landing tray, then guide it to the round exit. The cage is a real compound dynamic body."),
                Mechanism(ContainerShape.PendulumGate,"Between the beats","Set the pendulum swinging. Leave the waiting pocket when the passage clears.",
                    "A weighted pendulum blocks a real aperture in the partition. Use sideways rotation to swing it clear, then feed the ball through using the other tilt axis. Stabilizing the pendulum at an open angle is a valid physical solution. Missed timing is recoverable on the enclosure floor."),
                Mechanism(ContainerShape.FlightCatch,"Catch me","Build speed on the ramp. Rotate the receiving trough under the flying ball.",
                    "Gravity and rolling contact accelerate the steel ball down the takeoff ramp. During the unsupported flight the ball keeps its world-space trajectory while the player reorients the enclosure and receiving trough. Land in the receiving route and reach the real circular exit. A missed catch returns through the recovery route; no launch impulse or airborne attraction is added."),
                Mechanism(ContainerShape.MechanicalMemory,"The box remembers","Push the rack, return, push again. The ratchet remembers each physical stroke.",
                    "The ball physically compresses a spring-return rack, transferring contact force into a rotating cam. A retaining pawl constraint prevents backward motion after each reached tooth; a bounded rack stroke prevents skipping several stages. Read the physical cam and path position, repeat strokes, then cross the opened route. No timer or invisible trigger rotates the cam."),
                Mechanism(ContainerShape.MechanicalHeart,"Mechanical heart","A raises the bridge. B secures the route. Bring BOTH balls through the heart.",
                    "Two steel balls cooperate through a lever bridge, a weighted inner cage and a freefall receiving route inside a transparent spherical enclosure. A loads the far lever arm, B travels through the raised route and secures its retaining catch, then both leave through the inner transfer and final round hole. All required balls must physically exit.")
            };
        }

        private static PhysicsLabLayout Mechanism(ContainerShape shape,string title,string hint,string solution)
            => new PhysicsLabLayout { Shape=shape,Title=title,Hint=hint,Solution=solution };
