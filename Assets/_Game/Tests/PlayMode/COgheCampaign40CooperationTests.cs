...
namespace GravityBox.Tests
{
    ...
    public sealed class COgheCampaign40CooperationTests
    {
        ...
        [UnityTearDown] public IEnumerator After()
        {
            ...
            VenomCampaignSave.PersistenceEnabled = previousPersistence; yield return null;
        }
        ...
        private IEnumerator WaitFor(float seconds, Func<bool> condition, string why)
        {
            ...
            if (!condition()) COgheExpansionIntegrationTests.Capture(game, $"campaign40-{game.Definition.Order:00}-failed");
        ...
        private IEnumerator Pull(int anchor, COgheRailSlider rail, float direction, Func<bool> finished, float seconds = 35)
        {
            ...
            for (int i = 0; i < Mathf.CeilToInt(seconds / Dt) && !finished() && !game.Owner.Lost; i++)
            {
                if (!game.Attached)
                {
                    COgheExpansionIntegrationTests.Capture(game, $"campaign40-{game.Definition.Order:00}-grasp-failed");
                    Assert.Fail($"Lost the single grasp on {rail.name} after {i * Dt:F2}s; {State()}");
                }
                if (i % 90 == 0) input.Tap(game, target, false);
                Tick(); if (i % 240 == 0) yield return null;
            }
            if (!finished()) COgheExpansionIntegrationTests.Capture(game, $"campaign40-{game.Definition.Order:00}-pull-failed");
            game.ReleaseProp();
        ...
        private IEnumerator Leave()
        {
            ...
            COgheExpansionIntegrationTests.Capture(game, $"campaign40-{game.Definition.Order:00}-solution-before-exit");
            int anchor = Parts()[0]; game.SelectFragment(anchor);
            COgheExpansionIntegrationTests.Capture(game, "before-exit");
            ...
            COgheExpansionIntegrationTests.Capture(game, $"campaign40-{game.Definition.Order:00}-solution-complete");
        ...
        [UnityTest] public IEnumerator Audit35AlternateRecoveryTallPortrait() {yield return Solution35(true);}
        ...
        [UnityTest] public IEnumerator Audit36AlternateRecoveryTallPortrait() {yield return Solution36(true);}
        ...
        [UnityTest] public IEnumerator Audit37AlternateRecoveryTallPortrait() {yield return Solution37(true);}
        ...
        [UnityTest] public IEnumerator Audit38AlternateRecoveryTallPortrait() {yield return Solution38(true);}
